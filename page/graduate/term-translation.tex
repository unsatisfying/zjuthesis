...
\begin{center}
    ...
    % \begin{longtable}{m{2cm}<{\centering}m{8cm}<{\centering}m{5cm}<{\centering}} 
        ...
        IR&Intermediate Representation&中间语言表示\\
        CUDA&Compute Unified Device Architecture&统一计算设备架构\\
        SM&Streaming Multiprocessors&流式多处理器\\
        GMMU&GPU Memory Management Unit&GPU 内存运算单元\\
        GSP&GPU System Processor&GPU 系统处理器\\
        BFS&Breadth First Search&GPU 虚拟地址\\
        IOVA&Input-Output Virtual Address&输入输出虚拟地址\\
        IOMMU&Input–Output Memory Management Unit&输入输出内存管理单元\\
        PD3&Level-3 Page Directories&顶级页目录页表\\
        IAAS&Inference-As-A-Service&推理即服务\\
