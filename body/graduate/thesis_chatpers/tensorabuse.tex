...
\begin{figure}[!t]
    ...
    \label{fig:api-tf-versions}
\end{figure}

此外，本研究还分析了这些 API 在不同 TensorFlow 版本中的分布存在情况，如~\ref{fig:api-tf-versions}所示。分析显示，自 TensorFlow v1.0.0 的最早版本显示有 9 个 API 可以被滥用。随着 TensorFlow 的发展，其开发者开发者退出了更多的 API 以日益增长的功能需求，包括引入了更多与网络传输相关的 API，这使得 TensorFlow API 滥用的途径进一步扩大。这些结果突出了 TensorAbuse 的持久性和隐蔽性，表明它几乎跨越了所有 TensorFlow 版本。此外，由于相同攻击原语内不同的 API 可以互换，这也增加了这类攻击的灵活性和难以检测性。

本研究向谷歌、Huggingface Hub 和 ModelScan 等开发者报告了有关 TensorAbuse 攻击的问题。谷歌邮件回复中承认了这个问题，他们认为用户应该通过在沙箱中运行模型来降低这些风险，因此他们不会解决这个问题~\cite{google_code_sandboxing}。然而，Hugging Face Hub 和 ModelScan 尚未做出回应。在查看 ModelScan 的源代码时，本研究发现它只检查 TensorFlow 模型中的两个API，即 \code{ReadFile} 和 \code{WriteFile}。但是本研究可以通过扩展该工具的黑名单来帮助提高 ModelScan 工具的检测能力。此外本研究还将相关攻击上报至 Huntr 赏金漏洞网站，获得了其 1650 美元的奖励~\cite{huntr_platform}。
\section{本章小结}
