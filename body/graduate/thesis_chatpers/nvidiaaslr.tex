...
接着在地址搜集阶段，AnchorTrace 开始内存图遍历获取随机化地址。具体来说，本研究基于一个关键观察：内存图中大部分内存区域会在 ASLR 作用下随机化，但是有一个特定区域（图~\ref{fig:anchortrace} 中的内存区域\ding{182}）在多次分析内存图时其都保持不变。因此 AnchorTrace 将该内存区域作为可靠的锚点，并从该锚点（内存区域\ding{182}）开始，AnchorTrace 先从偏移 \code{0xb8} 和 \code{0xc0} 处分别获得系统常量段地址和 \code{.text} 段地址。然后根据偏移 \code{0x88} 处的指针走向另一个内存区域，该内存区域又包含指向全局栈内存和堆内存段的指针。最后通过依次类推的方式遍历这一内存图，可以递归地恢复所有内存段的地址。
